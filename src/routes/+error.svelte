	...
	<div class="max-w:600 w:full bg:white r:16 shadow:0|4|12|rgba(0,0,0,0.1) p:32">
		...
		{#if import.meta.env.DEV && $page.error}
			<details class="bg:yellow-50 rounded:8 p:16 mb:24">
				<summary class="font:14 font:bold fg:yellow-800 cursor:pointer">
					開発者向け情報
				</summary>
				<pre class="font:12 fg:gray-700 overflow:auto mt:8 white-space:pre-wrap">{JSON.stringify($page.error, null, 2)}</pre>
			</details>
		{/if}
